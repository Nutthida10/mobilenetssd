...
lineaccesstoken = 'nQaQzlDhJj4xLGlyc6BDoVbyyW/W8T53zCECg/Jr1W2Gk1ufc8Lhw+VdmSBN0tHeObz33qyQaSbcasbzLLBtgZwleDmYCX8hFUKK4wmtDEigAD+cl3F+PbrrQqIJIwNj47dzZHgAikX0Uoz7AKxEkAdB04t89/1O/w1cDnyilFU= '
...
def event_handle(event):
    ...
    if msgType == "text":
        ...
        if (msg == "สวัสดี") :
           ...
           headers = request.headers json_headers = json.dumps({k:v for k, v in headers.items()}) json_line = request.get_json(force=False,cache=False) ''' json_line = json.dumps(json_line) decoded = json.loads(json_line) crl= pycurl.Curl() crl.setopt( crl.URL, "https://bots.dialogflow.com/line/k--jomf/webhook") crl.setopt( crl.POST, 1) crl.setopt( crl.BINARYTRANSFER, true) crl.setopt( crl.POSTFIELDS, decoded) crl.setopt( crl.HTTPHEADER, json_headers) crl.setopt( crl.SSL_VERIFYHOST, 2) crl.setopt( crl.SSL_VERIFYPEER, 1) crl.setopt( crl.FOLLOWLOCATION, 1) crl.setopt( crl.RETURNTRANSFER, 1) crl.perform() crl.close() ''' replyObj = TextSendMessage(text=json_headers) line_bot_api.reply_message(rtoken, replyObj)
    elif msgType == "image":
        try:
